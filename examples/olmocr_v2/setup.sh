...
for CATEGORY in "${CATEGORIES[@]}"; do
  ...
  cargo run --release -- map \
    --input-dir "/mnt/raid0/s2pdf_dedupe_minhash_v1_with_no_pii_basic_quality_datadelve/${CATEGORY}/documents" \
    --output-dir "/mnt/raid0/s2pdf_dedupe_minhash_v1_with_no_pii_basic_quality_datadelve_norefs_mdtables_v2/${CATEGORY}/" \
    --config ./examples/olmocr_v2/config.yaml
done
