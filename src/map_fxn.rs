...
impl DataProcessor for FastTextAnnotator {
    fn new(config: &Value) -> Result<Self, Error> {
        ...
        let max_text_length = get_default(config, "max_text_length", 0 as usize);
    ...
    fn process(&self, mut data: Value) -> Result<Option<Value>, Error> {
        ...
        if self.max_text_length > 0 {
            // truncate text if max text length is specified as option
...
impl DataProcessor for UltrafinewebAnnotator {
    fn new(config: &Value) -> Result<Self, Error> {
        ...
        let max_text_length = get_default(config, "max_text_length", 0);
