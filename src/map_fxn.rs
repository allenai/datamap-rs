...
use crate::pyscore::run_ruff_on_string;
...
impl DataProcessor for RuffLinterAnnotator {
    ...
    fn process(&self, mut data: Value) -> Result<Option<Value>, Error> {
        let text = json_get(&data, &self.text_field).unwrap().as_str().unwrap().to_string();
        let score_result = run_ruff_on_string(&text).unwrap();

        let json_result = json!({"score": score_result.score, 
                                 "comment_score": score_result.comment_score, 
                                 "statements": score_result.total_statements, 
                                 "comments": score_result.total_comments});
