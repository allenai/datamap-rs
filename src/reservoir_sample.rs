...
fn unweighted_reservoir(
    ...
    let full_res: Vec<(Vec<Value>, usize)> = chunks_targets
        .into_par_iter()
        .map(|(pvec, target_size)| thread_res(&pvec, key, target_size, &pbar).unwrap())
        .collect();
    ...
    println!(
        "Made a reservoir of size {:?} from {:?} documents total",
        full_res.len(),
        total_seen
    );
...
fn get_chunks_targets(
    all_paths: Vec<PathBuf>,
    reservoir_size: usize,
) -> Result<Vec<(Vec<PathBuf>, usize)>, Error> {
    ...
    let chunks_targets = chunks
        .into_par_iter()
        .map(|pvec| {
            let chunk_size: usize = pvec
                .iter()
                .map(|p| fs::metadata(p).unwrap().len() as usize)
                .sum();
            let target_size =
                ((reservoir_size as f64) * ((chunk_size as f64) / (total_size as f64))) as usize;
            (pvec, target_size)
        })
        .collect();
    Ok(chunks_targets)
...
fn token_weighted_reservoir(
    ...
    for item in full_res.into_iter() {
        ...
        let percentile =
            ((cum_weight as f64 - item.weight as f64 / 2.0) / total_weight as f64) * 100.0;
        percentiles.push(json!({"percentile": percentile, "value": item.value}));
    }

    println!(
        "Made a reservoir of size {:?} from {:?} tokens total",
        percentiles.len(),
        total_weight
    );
