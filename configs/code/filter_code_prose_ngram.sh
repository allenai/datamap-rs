...
for source in "${SOURCES[@]}"; do
    ...
    input_dir="pretraining-data/sources/${source}_code_prose_tagged_partitioned_decon"
    output_dir="pretraining-data/sources/${source}_code_prose_tagged_partitioned_decon_ngram_filter"
    ...
    for partition in $(ls --color=never "${local_input_dir}"); do
        if [[ ! "${partition}" =~ ^quality_p([0-9]{2})$ ]]; then
            continue
        fi
