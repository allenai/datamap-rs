...
# Map commitpack language name to config filename (without .yaml)
get_config_name() {
    case "$1" in
        "c#") echo "csharp" ;;
        "c++") echo "cpp" ;;
        "objective-c++") echo "objective_cpp" ;;
        *) echo "${1//-/_}" ;;
    esac
}
...
# ============================================================================
# Download classifier models referenced in config files
# ============================================================================

# Track downloaded model dirs to avoid redundant syncs
declare -A DOWNLOADED_MODELS=()

for language in "${LANGUAGES[@]}"; do
    config_name=$(get_config_name "$language")
    config_file="${CONFIGS_DIR}/${config_name}.yaml"

    # Extract all fast_text_file paths from the config and download their directories
