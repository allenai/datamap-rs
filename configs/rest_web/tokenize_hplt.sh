...
BASE_DIR="pretraining-data/sources/hplt-project/three/sorted/eng_Latn/ia_quality_classified_v2_decon_topics_quality_partitioned"
...
ALL_SOURCES=(
    ...
    "crime_and_law"
    "education_and_jobs"
    "electronics_and_hardare"   # typo is expected
    "entertainment"
    "fashion_and_beauty"
...
for source in "${SOURCES[@]}"; do
    ...
    for step_dir in $(ls --color=never "${local_input_dir}"); do
        if [[ "${step_dir}" != vigintile_00* ]]; then
            echo "Skipping ${step_dir} as it does not start with vigintile_00"
            continue
        fi
